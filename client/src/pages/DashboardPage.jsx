...
function DashboardPage() {
    ...
    const handleOpen = (id) => navigate(`/builder/${id}`);
        ...
        <div className="dashboard">
            <div className="dashboard-header">
                <div>
                    ...
                    <p className="dashboard-subtitle">{projects.length} project{projects.length !== 1 ? 's' : ''}</p>
                </div>
                <button className="dashboard-new-btn" onClick={handleNewProject}>+ New Project</button>
            </div>
            {projects.length === 0 ? (
                <div className="dashboard-empty">
                    ...
                    <button className="dashboard-new-btn" onClick={handleNewProject}>+ Create First Project</button>
                </div>
            ) : (
                <div className="dashboard-grid">
                    {projects.map((project) => (
                        <ProjectCard key={project._id} project={project} onOpen={handleOpen} onDelete={handleDelete} />
                    ))}
                </div>
            )}
        </div>
